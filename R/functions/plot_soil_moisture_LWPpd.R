...
plot_soil_moisture_LWPpd <- function(){
  ...
  #- read in the TDR data 
  dfr <- data.frame(data.table::fread("data/WTC_TEMP_CM_TDRVWC_2013-03-01_2014-05-25_L1.csv"))
  dfr$DateTime <- as.POSIXct(dfr$DateTime,format="%Y-%m-%d %T",tz="GMT")
  dfr$date <- as.Date(dfr$DateTime)
  
  #exclude data prior to 1 Dec 2013
  dfr4 <- subset(dfr,date>as.Date("2013-12-01"))
  
  
  #calculate daily means, then treatment means
  dfr.day <- summaryBy(.~date+chamber+T_treatment+Water_treatment,data=dfr4,FUN=mean,keep.names=T,na.rm=T)
  dfr.day.treat <- summaryBy(.~date+T_treatment+Water_treatment,data=dfr.day,FUN=c(mean,se),keep.names=T,na.rm=T)
  dfr.day.treat$combotreat <- paste(dfr.day.treat$T_treatment,dfr.day.treat$Water_treatment,sep="_")
